...
class TestSample(TestCase):
    def setUp(self):
        ...
        product2 = Product.objects.create(category=category, name="p2", price=Decimal("11.0909"), stock=5)
        OrderItem.objects.create(order=order, product=product, quantity=2)
        OrderItem.objects.create(order=order, product=product2, quantity=3)
        OrderItem.objects.create(order=order2, product=product2, quantity=50)
